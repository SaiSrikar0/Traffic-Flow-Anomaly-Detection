...
with tab2:
    ...
    if len(numeric_cols) >= 2:
        ...
        if plot_type == "Scatter":
            ax.scatter(normal[feat1], normal[feat2], c='whitesmoke', edgecolors='black', linewidths=0.5, label='Normal', alpha=0.7, s=20)
            ax.scatter(anomaly[feat1], anomaly[feat2], c='red', label='Anomaly', alpha=0.9, s=100, marker='x', linewidths=2)
        else:
            ax.hist2d(normal[feat1], normal[feat2], bins=30, alpha=0.7, label='Normal', cmap='Greys')
    ...
    if len(numeric_cols) >= 3:
        ...
        fig.add_trace(go.Scatter3d(
            ...
            marker=dict(size=4, color='white', opacity=0.7, line=dict(color='black', width=0.5))
        ))
...
with tab3:
    ...
    ax.barh(var_df['Feature'], var_df['Variance_Diff'], color='darkred', edgecolor='black', linewidth=1)
